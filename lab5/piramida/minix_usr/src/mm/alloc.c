 ...
 */
  ...
  register struct hole* hp = NIL_HOLE;	
  register struct hole* prev_ptr = NIL_HOLE;	
  register struct hole* biggest = NIL_HOLE;	
  register struct hole* biggest_prev = NIL_HOLE;	
  ...
  if (IS_WORST_FIT){
    do {
      ...
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
        if (hp->h_len >= clicks && (biggest == NIL_HOLE || hp->h_len > biggest->h_len)) {
          biggest = hp;
...
{
  phys_clicks size = mm_in.m1_i1;
  phys_clicks* buffer = (phys_clicks*)mm_in.m1_p1;
  phys_clicks tmp_buf[2];
  ...
  while (hp != NIL_HOLE && hp->h_base < swap_base && size >= 3) {
    tmp_buf[0] = hp->h_len;
    tmp_buf[1] = hp->h_base;
    sys_copy(MM_PROC_NR, D, (phys_bytes) tmp_buf,
			who, D, (phys_bytes) buffer,
			(phys_bytes) 2 * sizeof(phys_clicks));
    buffer += 2;
    /* printf("WROTE %d\n",r); */
...
{
  IS_WORST_FIT = mm_in.m1_i1;
  /* printf("IS_WORST_FIT: %d\n", IS_WORST_FIT); */
  return 0;
}
