...
/* MODIFIED START */
FORWARD _PROTOTYPE( int do_setpercenta, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      /* MODIFIED START */
      case SYS_SETPERCENTA: r = do_setpercenta(&m); break;
 ...
 *				do_setpercenta				     *
 *===========================================================================*/
PRIVATE int do_setpercenta(m_ptr)
message *m_ptr;			/* pointer to request message */
{
  int percent_a = m_ptr->m_u.m_m1.m1i1;
  int percent_b = 100 - percent_a;
  int x = percent_a;
  int y = percent_b;
  int r;

  if (percent_a < 1 || percent_a > 99){
    return EDOM;
  }
  ...
  /* y = gcd(percent_a, percent_b) */

  TIMES_A = percent_a / y;
  TIMES_B = percent_b / y;

  return TIMES_A*100+TIMES_B;
}
...
message *m_ptr;			/* pointer to request message */
{
  int group = m_ptr->m_u.m_m1.m1i1;
  struct proc* process = proc_addr(m_ptr->m_u.m_m1.m1i2);
  if (group != 0 && group != 1){
    return EDOM; 
  }
  process->group = group;
...
message *m_ptr;			/* pointer to request message */
{
  struct proc* process = proc_addr(m_ptr->m_u.m_m1.m1i1);
