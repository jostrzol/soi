 ...
 */
  ...
  /*if (strcmp(rp->p_name, "proc_a") == 0 || strcmp(rp->p_name, "proc_b") == 0){
    printf("Pick proc: %s\n", rp->p_name);
  }*/
	proc_ptr = rp;
	bill_ptr = rp;
 ...
 */
  ...
  rdy_head[USER_Q] = rp;
  /* MODIFIED START */
  PROC_SCHED_COUNTER = rp->group == GROUP_A ? TIMES_A : TIMES_B;
  /* MODIFIED END */
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
    ...
    /* MODIFIED START */
    PROC_SCHED_COUNTER = xp->group == GROUP_A ? TIMES_A : TIMES_B;
    /* MODIFIED END */
#if (CHIP == M68000)
		if (rp == proc_ptr)
#endif
 ...
 */
  ...
  if (--PROC_SCHED_COUNTER == 0){
    ...
    PROC_SCHED_COUNTER = rdy_head[USER_Q]->group == GROUP_A ? TIMES_A : TIMES_B;
  }
  /* MODIFIED END */
  pick_proc();
}
 ...
 *				init_proc_group				    *
 *==========================================================================*/
PUBLIC void init_proc_group(rp)
struct proc *rp;
{
  rp->group = PROC_GROUP_INIT;
  if (--PROC_GROUP_INIT_COUNTER == 0){
    if (PROC_GROUP_INIT == GROUP_A){
      PROC_GROUP_INIT = GROUP_B;
      PROC_GROUP_INIT_COUNTER = PROC_GROUP_INIT_B_COUNT;
    }
    else {
      PROC_GROUP_INIT = GROUP_A;
      PROC_GROUP_INIT_COUNTER = PROC_GROUP_INIT_A_COUNT;
    }
  }
}
/* MODIFIED END */
